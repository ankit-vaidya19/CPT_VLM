...
class NolaConfig(LoraConfig):
    ...
    nola_num_basis: int = field(
        default=128, metadata={"help": "Number of basis in NOLA."}
    )

    def __post_init__(self):
        self.peft_type = PeftType.NOLA


class NolaModel(torch.nn.Module):
    """
    ...
    **Attributes**:
    ...
    def add_adapter(self, adapter_name, config=None):
        if config is not None:
            ...
            config = self._prepare_nola_config(config, model_config)
            self.peft_config[adapter_name] = config

        self._find_and_replace(adapter_name)
        if len(self.peft_config) > 1 and self.peft_config[adapter_name].bias != "none":
            raise ValueError(
                "NolaModel supports only 1 adapter with bias. When using multiple adapters, set bias to 'none' for all adapters."
            )
    ...
    def _check_target_module_exists(self, nola_config, key):
        if isinstance(nola_config.target_modules, str):
            target_module_found = re.fullmatch(nola_config.target_modules, key)
        else:
            target_module_found = any(
                key.endswith(target_key) for target_key in nola_config.target_modules
            )
            is_using_layer_indexes = (
                getattr(nola_config, "layers_to_transform", None) is not None
            )
            layer_indexing_pattern = getattr(nola_config, "layers_pattern", None)
            ...
            if is_using_layer_indexes and target_module_found:
                ...
                for pattern in layers_pattern:
                    ...
                    if layer_index is not None:
                        ...
                        if isinstance(nola_config.layers_to_transform, int):
                            target_module_found = (
                                layer_index == nola_config.layers_to_transform
                            )
                        else:
                            target_module_found = (
                                layer_index in nola_config.layers_to_transform
                            )
    ...
    def _create_new_module(self, nola_config, adapter_name, target):
        bias = hasattr(target, "bias") and target.bias is not None
        kwargs = {
            "use_nola": nola_config.use_nola,
            "nola_num_basis": nola_config.nola_num_basis,
            "r": nola_config.r,
            "lora_alpha": nola_config.lora_alpha,
            "lora_dropout": nola_config.lora_dropout,
            "fan_in_fan_out": nola_config.fan_in_fan_out,
            "init_lora_weights": nola_config.init_lora_weights,
        }
        loaded_in_4bit = getattr(self.model, "is_loaded_in_4bit", False)
        loaded_in_8bit = getattr(self.model, "is_loaded_in_8bit", False)
        ...
        if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
            ...
            if isinstance(target, torch.nn.Linear):
                ...
                if kwargs["fan_in_fan_out"]:
                    ...
                    kwargs["fan_in_fan_out"] = nola_config.fan_in_fan_out = False
                ...
                if not kwargs["fan_in_fan_out"]:
                    ...
                    kwargs["fan_in_fan_out"] = nola_config.fan_in_fan_out = True
            else:
                raise ValueError(
    ...
    def _find_and_replace(self, adapter_name):
        nola_config = self.peft_config[adapter_name]
        self._check_quantization_dependency()
        is_target_modules_in_base_model = False
        key_list = [key for key, _ in self.model.named_modules()]

        for key in key_list:
            if not self._check_target_module_exists(nola_config, key):
                continue
            ...
            if isinstance(target, LoraLayer) and isinstance(target, torch.nn.Conv2d):
                ...
                target.update_layer_conv2d(
                    adapter_name,
                    nola_config.r,
                    nola_config.lora_alpha,
                    nola_config.lora_dropout,
                    nola_config.init_lora_weights,
                )
                ...
                target.update_layer(
                    adapter_name,
                    nola_config.use_nola,
                    nola_config.nola_num_basis,
                    nola_config.r,
                    nola_config.lora_alpha,
                    nola_config.lora_dropout,
                    nola_config.init_lora_weights,
                ...
                new_module = self._create_new_module(nola_config, adapter_name, target)
                self._replace_module(parent, target_name, new_module, target)

        if not is_target_modules_in_base_model:
            raise ValueError(
                f"Target modules {nola_config.target_modules} not found in the base model. "
    ...
    @staticmethod
    def _prepare_nola_config(peft_config, model_config):
...
class GenerateParamsChunks(torch.autograd.Function):
    ...
    def forward(ctx, coefficients, chunks, out_dim, in_dim, seed, scale):
        ...
        params = torch.autograd.Variable(
            torch.tensor([chunks, out_dim, in_dim, seed, scale])
        )
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(chunks_length):
            ...
            W_ = W_ * (scale / math.sqrt(num_basis))
            W_ = W_.permute(1, 2, 0)
            grad_coefficients = torch.cat(
                (
                    grad_coefficients,
                    torch.einsum(
                        "d,dl->l", grad_output.flatten(), W_.reshape(-1, chunks_length)
                    ),
                ),
                dim=0,
            )
...
class NOLALinear(nn.Linear):
    # NOLA Linear Layer
    def __init__(self, in_features: int, out_features: int, num_basis=256, **kwargs):
    ...
    def forward(self, x: torch.Tensor):
        W = (
            self.generate_params(
                self.coefficients, self.out_features, self.in_features, self.seed
            )
            + self.weight
        )
        return x @ W.t()
        ...
        if r > 0:
            if use_nola:
                self.lora_A.update(
                    nn.ModuleDict(
                        {
                            adapter_name: NOLALinear(
                                self.in_features,
                                r,
                                num_basis=nola_num_basis,
                                bias=False,
                            )
                        }
                    )
